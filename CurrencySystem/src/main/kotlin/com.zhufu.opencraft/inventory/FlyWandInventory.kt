...
class FlyWandInventory(val player: Player, plugin: Plugin) : ClickableInventory(plugin) {
    ...
    override val inventory: Inventory = Bukkit.createInventory(null,InventoryType.CHEST, TextUtil.info("续费飞行法杖[uuid:${++id}}]"))
    ...
    val wand = if (FlyWand.isThis(player.inventory.itemInMainHand)) FlyWand(
        player.inventory.itemInMainHand,player.lang()
    ) else null
    var time = 1
                ...
                PaymentDialog(player, SellingItemInfo(confirmItem.clone(),100,time),TradeManager.getNewID(),plugin)
                        .setOnConfirmListener {
                            ...
                            if (info.currency >= price){
                                info.currency -= price
                                player.inventory.setItem(player.inventory.heldItemSlot, wand!!.also { meta -> meta.updateTime(wand.timeRemaining+time*60) })

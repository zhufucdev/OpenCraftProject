...
class VisitorInventory(plugin: Plugin,val player: Player)
    : PageInventory<VisitorInventory.VisitorAdapter>(TextUtil.getColoredText("浏览商家[uuid:$id]",TextUtil.TextColor.RED,false,false),
        VisitorAdapter(),
        54,
        plugin) {
                    ...
                    .also {
                        it.itemMeta = (it.itemMeta as SkullMeta)!!.also { meta ->
                            ...
                            element.forEach { info ->
                                lore.add(TextUtil.getColoredText(
                                        info.items!!.item.type.name
                                        + "×"  + info.items!!.amount
                                        + "->" + info.items!!.prise,

...
object GameManager {
        ...
        if (result == GameBase.JoinGameResult.ROOM_FULL){
            return joinPlayerTo(
                    getNewID().also { id ->
                        mList.add(
                                (Class.forName("com.zhufu.opencraft.games.$name").newInstance() as GameBase)
                                        .also { it.initGame(id, player, plugin!!) }
                        )
                    }
                    , player)
        }
    ...
    fun joinPlayerTo(id: Int,player: Player): GameBase.JoinGameResult = mList.firstOrNull { it?.gameID == id }?.joinPlayer(player)?:GameBase.JoinGameResult.CANCELLED

    private fun maxID(): Int{
        var max = -1
        mList.forEach {
            if(it != null && it.gameID > max){
                max = it.gameID
            }
        }
        return max
    ...
    fun forEach(action: (GameBase) -> Unit) = mList.forEach{ if (it != null) action(it) }

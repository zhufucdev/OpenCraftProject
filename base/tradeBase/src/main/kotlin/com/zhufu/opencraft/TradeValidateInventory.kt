...
class TradeValidateInventory(val tradeInfo: TradeInfo): NPCItemInventory(
        tradeInfo.location!!,tradeInfo.items!!.item.clone().also { it.amount = tradeInfo.items!!.amount },
        TradeManager.plugin!!) {
        ...
        (clickableNPC.entity as ArmorStand)
                .apply {
                    setHelmet(ItemStack(Material.PLAYER_HEAD).apply {
                        itemMeta = (itemMeta as SkullMeta).also { meta -> meta.owningPlayer = Bukkit.getOfflinePlayer(UUID.fromString(tradeInfo.getSeller())) }
                    })
    ...
    private fun setShowingItem(){
        ...
        inventory.setItem(9*1+4,
                tradeInfo.items!!.item.clone().also {
                    ...
                    it.itemMeta = it.itemMeta!!.apply { lore = listOf(TextUtil.info("交换${amount}个，共${tradeInfo.items!!.amount}个"), TextUtil.tip("这将消耗您${tradeInfo.items!!.unitPrise * amount}个货币")) }
                }
        )
    }
    ...
    private fun init(){
        ...
        inventory.setItem(9*1+3,
                ItemStack(Material.NETHER_STAR)
                        .apply {
                            itemMeta = itemMeta!!.apply { setDisplayName(TextUtil.getColoredText("少交换一个", TextUtil.TextColor.GOLD,false,
                                underlined = false
                            )) }
                            subtractItem = this
                        }
        ...
        inventory.setItem(9*1+5,
                ItemStack(Material.NETHER_STAR)
                        .apply{
                            itemMeta = itemMeta!!.apply { setDisplayName(TextUtil.getColoredText("多交换一个", TextUtil.TextColor.GOLD,false,
                                underlined = false
                            )) }
                            plusItem = this
                        }
        ...
        inventory.setItem(inventory.size -1,
                Widgets.confirm.apply{
                            itemMeta = itemMeta!!.apply { setDisplayName(TextUtil.getColoredText("确认", TextUtil.TextColor.GREEN, true,
                                underlined = false
                            )) }
                            confirmItem = this
                        }
        ...
        if (inventoryName.startsWith("修改")){
            ...
            if (amount != 0) {
                if (TradeManager.checkLimit(tradeInfo.items!!.unitPrise,amount)){
                    player.inventory.addItem(tradeInfo.items!!.item.also { it.amount = amount - tradeInfo.items!!.amount })
    ...
    private fun confirm(player: HumanEntity){
        isPaying = true
        PaymentDialog(player, tradeInfo.items!!.clone().also { it.amount = amount }, tradeInfo.id, TradeManager.plugin!!)

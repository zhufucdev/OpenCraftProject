...
class PlayerSelf private constructor(private val serverInfo: ServerPlayer, private val executor: Scripting.Executor){
    ...
    val location get() = ease { SimpleLocation.from(location) }
    ...
    val entity get() = ease { SimpleEntity.from(this) }

    val level get() = ease { level }
    val foodLevel get() = ease { foodLevel }
    val spawnpoint
        get() = serverInfo.tag.getSerializable("surviveSpawn", Location::class.java, null).let {
            if (it != null)
                SimpleLocation.from(it)
            else
                null
        }

...
class PlayerBlockOperation(player: String, time: Long, var block: Material? = null,var l: Location? = null,var type: BlockOperationType? = null) : OperationChecker.PlayerOperation(player, time) {
    enum class BlockOperationType{
        PLACE,BREAK
    }
    override val operationType: OperationChecker.OperationType
        get() = OperationChecker.OperationType.BLOCK
    ...
    override fun toLocalMessage(): String = "$player 于${format.format(Date(time))} ${if (type == BlockOperationType.PLACE) "放置" else "破坏"} 了位于 ${location?.world?.name}(${location?.blockX},${location?.blockY},${location?.blockZ}) 的 ${block?.name} 方块"
}
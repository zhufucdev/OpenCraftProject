...
class BlockLocker : JavaPlugin(), Listener{
    override fun onEnable() {
        ...
        ServerCaller["ChangeChunkOwner"] = {
            ...
            else throw IllegalStateException("Par@ChangeChunkOwner must not be null.")
        }
    }
            ...
            when(args.first()) {
                ...
                "unset" -> {
                    ...
                    if (name != "clear") {
                        ...
                        BlockLockManager.forEverything { if ((sender !is Player || it.owner == sender.uniqueId.toString() || sender.isOp) && it.accessible.remove(player.uniqueId)) removed.append("${it.name} ") }
                ...
                "check" -> {
                    ...
                    if (sender !is Player || sender.isOp || sender.uniqueId.toString() == info.owner) {
                        ...
                        if (map.isEmpty()) {
                            ...
                            sender.sendMessage(
                                    TextUtil.getColoredText(getter["block.statics.title",info.name], TextUtil.TextColor.GOLD, false, underlined = false))
                ...
                "rename" -> {
                    ...
                    when {
                        BlockLockManager.contains(args.first()) -> {
                                    ...
                                    when (name) {
                                        ...
                                        "<-" -> mode = 'D'
                                            ...
                                            if (info is BlockLockManager.BlockInfo)
                                                ...
                                                val cause = getter["block.grouping.reason",BlockLockManager.BlockInfo::class.simpleName,if (info == null)"null" else info::class.simpleName]
                                                warn.add(IllegalArgumentException(getter["block.grouping.giveUp",name],Throwable(cause)))
                                            }
                                        }
                                    }
                                } else {
                            ...
                            if (group == null){
                                if (groupName.isNotEmpty()) {
                                    ...
                                    new.owner = (sender as? Player)?.uniqueId?.toString() ?: getter["command.error.unknown"]
                                    group = new
                        ...
                        args.size in 5 .. 7 -> {
                            ...
                            (0..3).forEach { i ->
                                if (args[i].any { !it.isDigit() && it != '-' })
                                    nondigit.add(args[i])
                            }
                            if (nondigit.size != 0) {
                                sender.error(getter["block.error.notDigit",buildString { nondigit.forEach { append("$it,");deleteCharAt(lastIndex) } }])
                                return false
                            }
                            if (args[4] == "clear") {
                                sender.error(getter["block.error.illegalKeyword"])
                                return true
                            }
                            if (BlockLockManager.firstOrNull { it.name == args[4] } != null){
                                sender.error(getter["block.error.name",args[4]])
                                return true
                            }

                            val from = BlockLockManager.XZ(args[0].toInt(), args[1].toInt())
                            val to = BlockLockManager.XZ(args[2].toInt(), args[3].toInt())
                            val blocks = (Math.abs(from.x - to.x) + 1)*(Math.abs(from.z - to.z) + 1)
                            val prise = env.getLong("prisePerBlock")
                            val total = prise * blocks
                            val info = sender.info()
                            if (info == null){
                                sender.error(getter["player.error.unknown"])
                                return true
                            }
                            if (info.currency < total){
                                sender.error(getter["block.error.noEnoughCoin"])
                                sender.tip(getter["block.size",blocks,total,info.currency])
                                return true
                            }
                            PaymentDialog(sender,
                                    SellingItemInfo(
                                            ItemStack(Material.GRASS_BLOCK)
                                                    .also { itemStack ->
                                                        itemStack.itemMeta = itemStack.itemMeta!!.apply {
                                                            setDisplayName(TextUtil.getColoredText(getter["block.block"], TextUtil.TextColor.GREEN, false, false))
                                                        }
                                                    },
                                            prise,
                                            blocks)
                                    ...
                                    .setOnConfirmListener { dialog ->
                                        val t = BlockLockManager.firstOrNull { it.contains(from, to) }
                                        if (t != null) {
                                            sender.error(getter["block.error.duplicated",t.name])
                                        ...
                                        info.currency -= total
                                        sender.success(getter["block.success",args[4],"(${from.x},${from.z})-(${to.x},${to.z})",owner.name])
                                        sender.info(getter["user.coinRemaining",info.currency])
                                    }
                                    .setOnCancelListener {
